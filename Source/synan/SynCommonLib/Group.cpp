...
bool CGroups::change_words_in_group_gramcodes(const CPeriod &group, const std::string &gramcodes,
                                              GrammemCompare CompareFunc) {
    if (gramcodes[0] == 0) return false;
    const CAgramtab *R = GetOpt()->GetGramTab();
    bool isok = true;
    for (int i = group.m_iFirstWord; i <= group.m_iLastWord; i++) {
        std::string groupGramCodes = std::string(gramcodes);
        uint64_t grammems = R->ChangeGleicheAncode1(CompareFunc, std::string(sent[i].GetGramcodes()), groupGramCodes,
                                                    sent[i].GetGrammems());
        if (groupGramCodes == "") {
            isok = false;
            continue;
...
void CGroups::BuildAutomaticSynrels() {
    ...
    for (size_t GroupNo = 0; GroupNo < m_Groups.size(); GroupNo++) {
        ...
        } else if (G.m_GroupType == GetOpt()->m_PrepNounGroupType) {
            create_syn_rel(G, get_main_word_in_group(G),
                           get_main_word_in_group(get_maximal_subgroup(GroupNo, G.m_iLastWord)),
                           GetOpt()->m_PrepNounGroupType);
        } else if (G.m_GroupType == GetOpt()->m_DirObjGroupType) {
            create_syn_rel(G, get_main_word_in_group(G),
                           get_main_word_in_group(get_maximal_subgroup(GroupNo, G.m_iLastWord)),
                           GetOpt()->m_DirObjGroupType);
        } else if (G.m_GroupType == GetOpt()->m_NPGroupType) {
