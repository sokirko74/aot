...
bool CFormatCaller::try_and_step_forward(CFormatCall* FC, CGroup& G, int rule)
{
  ...
  if (   ((*this).*(FC->m_format))(G) )
  {
    ...
    { 
		...
		rml_TRACE("%s (%i, %i) (%s, %s) (%s , %s) %s is created\n", (*FC).m_name.c_str(), G.m_iFirstWord, G.m_iLastWord, //GetOpt()->GetGroupNameByIndex(G.m_GroupType)
...
int CFormatCaller::main_analyse()
{
	...
	catch(...)
	{
		...
		strMsg = convert_to_utf8(strMsg.c_str(), GetOpt()->m_Language);
		GetOpt()->OutputErrorString(strMsg);
	}
