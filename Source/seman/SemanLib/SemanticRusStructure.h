...
public:
    ...
    */
    ...
    bool CreateClauseRelation(long ClauseRuleNo, long ClauseNo1, long ClauseNo2, CValency V, long TargetNodeNo,
                              CDictUnitInterp Conj, long SourceNodeNo = -1);
    ...
    bool
    GetFreeActantPattern(long NodeNo, CSemPattern &P, CSynRealization &SynReal, std::string SynRel, bool CheckSynRel,
                         std::string SynFet, bool CheckSynFet);
...
//  выдает число операторов однородностей (MUA), у которых все члены имеют хотя бы одну	// общую  SF
    ...
    void GetThesInterps(std::string UnitStr, const CRusSemWord &W, const CThesaurus *Thes, DictTypeEnum DictType,
                        CRusSemNode &N) const;
    ...
    long SetBestVariant(const std::vector<TreeAndValueVector> &VarAndVals, const long HypotCount,
                        const long TopConnectedComponentsCount);
    ...
    long FindAlreadyBuiltClauseVariant(long ClauseNo) const {
        std::vector<CSemClauseVariantResult>::const_iterator It = find(m_AlreadyBuiltClauseVariants.begin(),
                                                                       m_AlreadyBuiltClauseVariants.end(),
                                                                       CSemClauseVariantResult(
                                                                               m_Clauses[ClauseNo].m_SyntaxClauseNo,
                                                                               m_Clauses[ClauseNo].m_ClauseVariantNo,
                                                                               m_Clauses[ClauseNo].m_AddedClauses));
