...
void CRusSemStructure::DelUnusedRelations(long ClauseNo)
{
	for (size_t i = 0; i < m_Relations.size(); i++)
		if (IsInClause(m_Relations[i].m_SourceNodeNo, ClauseNo))
			if (!m_Relations[i].m_bRelUse)
			{
				EraseRelation(i, "from DelUnusedRelations");
				i--;
			};
};
...
long CRusSemStructure::SetBestVariant(const std::vector<TreeAndValueVector>& VarAndVals,  const long HypotCount, const long TopConnectedComponentsCount) {
	std::vector<long> variants(TopConnectedComponentsCount * HypotCount, 0); // a matrix
	...
	for (size_t i = 0; i < TopConnectedComponentsCount; i++)
		AddTreeVariant(VarAndVals[i][variants[varIndex * TopConnectedComponentsCount + i]].first);
...
TryWithoutLongRelationsLabel:
	...
	assert(TopConnectedComponentsCount * HypotCount > 0);

	long treeVariantCount = SetBestVariant(VarAndVals, HypotCount, TopConnectedComponentsCount);
