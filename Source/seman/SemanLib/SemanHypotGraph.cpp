...
bool CRusSemStructure::InitValsRussian(long NodeNo) {
    ...
    if (m_Nodes[NodeNo].GetType() != NoneRoss) {
        ...
        if (UnitNo != ErrUnitNo)
            GetRossHolder(m_Nodes[NodeNo].GetType())->GetLexFuncts(UnitNo, m_Nodes[NodeNo].m_LexFunctFields, OborRoss,
                                                                   m_pData->GetRossHolder(OborRoss));
            ...
            for (int i = 0; i < P.m_ActantSemFets.size(); i++)
                for (int k = 0; k < P.m_ActantSemFets[i].size(); k++)
                    if (((std::string(P.m_ActantSemFets[i][k]) != "Copul")
                         ...
                         && !is_null(GetRossHolder(Ross)->GetItemNoByItemStr(P.m_ActantSemFets[i][k], "D_LF", false))
                        )
    ...
    FindAbstractAdditionArticle(Ross, m_Nodes[NodeNo], Articles, IsClauseSyntaxRoot,
                                m_Clauses[m_Nodes[NodeNo].m_ClauseNo].m_ClauseType);
    ...
    */
    std::vector<std::string>::const_iterator It = find(m_IndexedSemFets.begin(), m_IndexedSemFets.end(),
                                                       std::string("IDENT"));
        ...
        if (IsInClause(i, ClauseNo))
            if (m_Nodes[i].IsWordContainer())
                if (m_Nodes[i].m_MainWordNo != -1) {
                    ...
                    if ((m_Nodes[i].m_Words[m_Nodes[i].m_MainWordNo].GetTypeGrammems() & GrammemsSet[0]) ==
                        GrammemsSet[0])
                        return true;
...
void CRusSemStructure::BuildCircumsChains(long ClauseNo) {
    for (long NodeNo = m_Clauses[ClauseNo].m_BeginNodeNo; NodeNo < m_Clauses[ClauseNo].m_EndNodeNo; NodeNo++) {
        ...
        } else if (CheckAllIncomingRelations(NodeNo, "LOK")) {
