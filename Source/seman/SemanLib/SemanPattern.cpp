...
bool CRusSemStructure::CheckPatternGramFetLine(CSemPattern& P, CSynRealization& SynRealization, size_t NodeNo)
{
	...
	if (P.m_pRossDoc->IsPosition(C.GetItem(0)))
		if (!CheckDirection(P.m_SourceNo, NodeNo, P.m_pRossDoc->GetDomItemStrWrapper(C.GetItem(0))))
			return false;
	...
	if ((C.GetItem(0) == P.m_pRossDoc->SubjSynONo)
	...
	*/
	...
	dom_item_id_t SynRelNo = P.GetSynRel(SynRealization.m_CortegeNo);
	std::string SynRelStr = P.m_pRossDoc->GetDomItemStrWrapper(SynRelNo);
	dom_item_id_t SynFet = P.GetSynFet(SynRealization.m_CortegeNo);
	std::string SynFetStr = P.GetSynFetStr(SynRealization.m_CortegeNo);
