...
public:
	...
	const std::string& GetDictPath() const;

	bool LoadStructDict(const std::string strPathName, bool bDontLoadExamples, long last_update_time=1);

	dom_item_id_t GetItemNoByItemStr(const std::string& ItemStr, const char* DomStr, bool check = true) const;
	dom_item_id_t GetItemNoByItemStr1(const std::string& ItemStr, BYTE domNo, bool check = true) const;
	...
	bool HasBeenModified(const long T) const;
	void SetUpdateTime(long T);
	...
	void				GetLexFuncts(size_t UnitNo, std::vector<CLexicalFunctionField>& OutVector, DictTypeEnum type = NoneRoss, const CStructDictHolder* pRossHolderObor = NULL) const;
