...
void CBasicDomainEdit::OnAddButton() 
{
  ...
  {
      if (!InputBox ("New domain", OutBuffer, 100))
		  return;
  ...
  if (GetRoss()->m_Domens[m_DomNo].Source == (unsigned char)dsExpres)
  {
    ...
    if (!InputBox ("New sense", OutBuffer, 100))
		return;
	...
	while (Pos != -1)
	{
		...
		for (size_t i = 0;  i<GetRoss()->_GetCortegesSize(); i++)
			for (size_t k=0; k < MaxNumDom; k++)
				...
				{
					CString prompt;
					prompt.Format("Constant \"%s\" occurs %i times. Delete all occurrences?", (const char*)GetRoss()->GetDomItemStr(ItemNo), Refs.size());
