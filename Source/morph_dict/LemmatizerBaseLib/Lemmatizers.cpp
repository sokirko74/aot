...
bool CLemmatizer::LemmatizeWord(std::string &InputWordStr, const bool cap, const bool predict,
...
bool
CLemmatizer::GetAllAncodesAndLemmasQuick(std::string &InputWordStr, bool capital, char *OutBuffer, size_t MaxBufferSize,
...
void CreateDecartProduction(const std::vector<CFormInfo> &results1, const std::vector<CFormInfo> &results2,
...
bool CLemmatizer::CreateParadigmCollection(bool bNorm, std::string &InputWordStr, bool capital, bool bUsePrediction,
    ...
    if (!IsFound(Result)) // not found or predicted
    {
        ...
        if (GetLanguage() == morphRussian && InputWordStr.length() > 12 && hyph == std::string::npos)
            for (int n = 0; n < NumeralToNumberCount; n++) {
                ...
                if (NumeralToNumber[n].m_Ordinal[0] != 0 && InputWordStr.length() > len + 1
                    && (pos = InputWordStr.substr(InputWordStr.length() - len - 1).rfind(
                        std::string(NumeralToNumber[n].m_Ordinal).substr(0, len - 2))) != std::string::npos) {
        ...
        if (hyph != std::string::npos) {
            ...
            */
            ...
            else if (IsHyphenPrefix(first_part)) {
...
bool CLemmatizer::CheckAbbreviation(std::string InputWordStr, std::vector<CAutomAnnotationInner> &FindResults,
                                    bool is_cap) const {
...
void CLemmatizer::PredictByDataBase(std::string InputWordStr, std::vector<CAutomAnnotationInner> &FindResults,
                                    bool is_cap) const {
    ...
    if ((has_nps[0] == -1) // no noun
        || (is_cap && (GetLanguage() !=
                       morphGerman)) // or can be a proper noun (except German, where all nouns are written uppercase)
            ) {
        m_Predict.Find(m_pFormAutomat->GetCriticalNounLetterPack(), res);
