...
CRusFormatCaller::CRusFormatCaller(const CSyntaxOpt* Opt) : CFormatCaller(Opt) 
{
	...
	InsertRule ( rule_for_numbers, format_for_numbers, _R("КОЛИЧ"), FROM_LEFT);
	InsertRule ( rule_for_num_complex, format_for_num_complex,_R("СЛОЖ_ЧИСЛ"), FROM_LEFT);
	InsertRule ( rule_for_similar_numerals, format_for_similar_numerals,_R("ОДНОР_ЧИСЛ"), FROM_LEFT);
	InsertRule ( rule_for_noun_adjuncts, format_for_noun_adjunct_group,_R("ПРИЛОЖЕНИЕ"), FROM_LEFT);
	InsertRule ( rule_for_noun_num, format_for_noun_num,_R("СУЩ_ЧИСЛ"), FROM_LEFT);
	InsertRule ( rule_for_names, format_for_FAM1_FAM2, _R("ФИО"), FROM_LEFT);
	InsertRule ( rule_for_adv_adj, format_for_adv_adj,_R("НАР_ПРИЛ"), FROM_LEFT);
	InsertRule ( rule_for_similar_adjs, format_for_similar_adjs,_R("ОДНОР_ПРИЛ"), FROM_LEFT);
	InsertRule ( rule_for_modif_adj, format_for_modif_adj_groups,_R("МОДИФ_ПРИЛ"), FROM_LEFT);
	InsertRule ( rule_for_similar_advs, format_for_similar_advs,_R("ОДНОР_НАР"), FROM_LEFT);
	InsertRule ( rule_for_similar_advs_comp_conj, format_for_similar_advs_comp_conj,_R("ОДНОР_НАР"), FROM_LEFT);
	InsertRule ( rule_for_similar_infs, format_for_similar_infs,_R("ОДНОР_ИНФ"), FROM_LEFT);
	InsertRule ( rule_for_similar_comp_adjs, format_for_similar_comp_adjs,_R("ОДНОР_ПРИЛ"), FROM_LEFT);
	InsertRule ( rule_for_web_addr, format_for_web_addr,_R("ЭЛ_АДРЕС"), FROM_LEFT);
	InsertRule ( rule_for_comp_adv, format_for_comp_adv,_R("СРАВН_СТЕПЕНЬ"), FROM_LEFT);
	InsertRule ( rule_for_adv_verb, format_for_adv_verb,_R("НАРЕЧ_ГЛАГОЛ"), FROM_LEFT);
	InsertRule ( rule_for_noun_groups, format_for_noun_groups,_R("ПРИЛ_СУЩ"), FROM_LEFT);
	InsertRule ( rule_for_number_adverb, format_for_number_adverb,_R("НАР_ЧИСЛ_СУЩ"), FROM_LEFT);
	InsertRule ( rule_for_number_noun, format_for_number_noun,_R("ЧИСЛ_СУЩ"), FROM_LEFT);
	InsertRule ( rule_for_approx_noun_number, format_for_approx_noun_number,_R("АППРОКС_ИГ"), FROM_LEFT);
	 
	InsertRule ( rule_for_rank_surname, format_for_rank_surname,_R("ДОЛЖ_ФИО"), FROM_LEFT);
	InsertRule ( rule_for_gen_chains, format_for_gen_chains,_R("ГЕНИТ_ИГ"), FROM_LEFT);

	InsertRule ( rule_for_from_compar, format_for_from_compar,_R("ОТСРАВН"),FROM_LEFT);
	 
	InsertRule ( rule_for_preps, format_for_preps,_R("ПГ"), FROM_LEFT);
	InsertRule ( rule_for_prep_IZ, format_for_prep_IZ,"ПГ (из)", FROM_LEFT);
	 
	InsertRule ( rule_for_selective_groups, format_for_selective_groups,_R("ЭЛЕКТ_ИГ"), FROM_LEFT);
	InsertRule ( rule_for_similar_noun_groups, format_for_similar_noun_groups,_R("ОДНОР_ИГ"), FROM_LEFT);
	InsertRule ( rule_for_neg_verb,format_for_neg_verb,_R("ОТР_ФОРМА"), FROM_LEFT);
	InsertRule ( rule_for_dir_obj, format_for_dir_obj,_R("ПРЯМ_ДОП"), FROM_LEFT);
	InsertRule ( rule_for_instr_obj, format_for_instr_obj,_R("ИНСТР_ДОП"), FROM_LEFT);
	InsertRule ( rule_for_dir_obj_rev, format_for_dir_obj_rev,_R("ПРЯМ_ДОП_РЕВ"), FROM_LEFT);
	InsertRule ( rule_for_verb_inf, format_for_verb_inf,_R("ПЕР_ГЛАГ_ИНФ"), FROM_LEFT);
	InsertRule ( rule_for_adv_adv, format_for_adv_adv,_R("НАР_НАР"), FROM_LEFT);
	InsertRule ( rule_adv_number_adverb, format_for_adv_number_adverb,_R("НАР_НАР_ЧИСЛ"), FROM_LEFT);
	InsertRule ( rule_for_approx_prep_noun, format_for_approx_noun_prep,_R("АППРОКС_ПГ"), FROM_LEFT);
	InsertRule ( rule_for_partic_clause, format_for_partic_clause,_R("ПРИЧ_СУЩ"),FROM_RIGHT);
	InsertRule ( rule_for_whose_clause, format_for_whose_clause,_R("ПРИДАТ_ОПР"),FROM_RIGHT);
	InsertRule ( rule_for_adj_in_commas_preposition, format_for_adj_in_commas_preposition,_R("ПРИЛ-СУЩ"),FROM_RIGHT);
	InsertRule ( rule_for_plural_noun_adj, format_for_plural_noun_adj,_R("ПРИЛ-СУЩ"),FROM_LEFT);
	InsertRule ( rule_for_noun_adj_postposition, format_for_noun_adj_postposition,_R("ПРИЛ_ПОСТПОС"),FROM_LEFT);
	InsertRule ( rule_for_noun_detached_adj_postposition, format_for_noun_detached_adj_postposition,_R("СУЩ_ОБС_ПРИЛ"),FROM_LEFT);
	InsertRule ( rule_for_keyb, format_for_keyb,_R("КЛВ"),FROM_LEFT);
	InsertRule ( rule_for_disrupt_conj, format_for_disrupt_conj,_R("РАЗРЫВ_СОЮЗ"),FROM_LEFT);
	InsertRule ( rule_for_adv_predik, format_for_adv_predik,_R("НАР_ПРЕДИК"), FROM_LEFT);
	InsertRule ( rule_for_anat_compar, format_for_anat_compar,_R("АНАТ_СРАВН"), FROM_LEFT);
	InsertRule ( rule_for_foreign_term, format_for_foreign_term,_R("ИНОСТР_ЛЕКС"),FROM_LEFT);
	InsertRule ( rule_for_preps_adj, format_for_preps_adj,"ПГ (для прилагательных)", FROM_LEFT);
	InsertRule ( rule_for_oborots, format_for_oborots,_R("ОБОРОТ"), FROM_LEFT);
