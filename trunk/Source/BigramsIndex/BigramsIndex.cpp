...
bool BuildBigramsBin(string BigramsFileName, vector<CWordInfo> &WordInfos, bool bFirstOffset1) {
    ...
    while (fgets(buffer, 10000, in_fp)) {
        if ((BigramsCount % 100000) == 0)
            fprintf(stderr, "%zu               \r", BigramsCount);
        char w1[500], w2[500];
        int Bigramsfreq;
        ...
        if (curr_it == WordInfos.end()) {
            ...
            if (bFirstOffset1) {
                if (bChange && (curr_it->m_FileOffset1 != UINT_MAX)) {
                    fprintf(stderr, "Wrong order \"%s\" from line \"%s\" (curr_it->m_FileOffset1=%zu)\n", w, buffer,
                            curr_it->m_FileOffset1);
                    return false;
                }
...
#ifdef __unix__
        const char *sortCommand = "sort";
#else
        const char* sortCommand = "gsort"; //  cygwin
#endif
        string Command = Format("%s -k 2,3 <%s >%s", sortCommand, BigramsFileName.c_str(), RevFile.c_str());
        fprintf(stderr, "%s\n", Command.c_str());
